"""
Simple single-factor ("CAPM-like") next-month return scaling-law simulation.

Companion to ``simulation_example.py``. It exists to answer one question: do
neural-network scaling laws appear even when the TRUE data-generating process is
trivially simple? The worry is that scaling curves are partly mechanical and
will show up regardless of how much real structure the signal has.

The DATA side is held IDENTICAL to ``simulation_example.py``:

- the same 64 AR(1) firm-month characteristics (same seed -> same values),
- the same month-wide common shock and firm-level idiosyncratic noise,
- the same panel shape, columns, date split, and scaling-law settings.

The ONLY thing that changes is the conditional mean E[ret_{t+1} | chars_t]:

- ``simulation_example.py``: a complex function of ALL 64 characteristics
  (linear levels + nonlinear levels + pairwise + nonlinear interactions),
- this script: a single-characteristic LINEAR signal (``char_01`` only),
  rescaled to the SAME predictable variance as the complex signal. The other
  63 characteristics are irrelevant and statistically independent of returns.

How the identical-data guarantee works: we literally run the complex DGP first
(consuming the RNG exactly as the original does) and keep its conditional-mean
*variance*, then discard its *shape* and swap in the single-factor signal. Since
the characteristics and every noise draw come from the same untouched RNG
stream, the features and the noise realization match the original economy
byte-for-byte; only the mapping characteristics -> expected return changes.

Because a one-variable linear regression is the Bayes-optimal predictor here, a
small model already captures everything there is to learn. Any rise in test R^2
as model size / compute grows is therefore mechanical rather than a reflection
of a genuinely learnable complex signal -- which is exactly what this script
lets you check, holding the R^2 ceiling equal to the complex run.

Outputs are written to:

    ~/Desktop/simple single factor next return simulation
"""
...
# Hidden expected-return controls for the COMPLEX signal. These are identical to
# simulation_example.py and are still generated here in full -- not because the
# complex signal is used, but because generating it (a) keeps the RNG stream
# identical so the characteristics and noise match the original economy exactly,
# and (b) fixes the predictable variance the simple signal is rescaled to match.
...
# ---------------------------------------------------------------------------
# SIMPLE-DGP controls. This is the only conceptual difference from the complex
# example: the conditional mean depends on ONE characteristic, linearly.
# ---------------------------------------------------------------------------
SIGNAL_CHAR_INDEX = 0  # Which characteristic is the single factor (0 -> char_01).
SIGNAL_STRENGTH_MULTIPLIER = 1.0  # 1.0 = exactly match the complex signal's variance
#                                   (same R^2 ceiling); >1 stronger, <1 weaker.

OUTPUT_DIR = Path.home() / "Desktop" / "simple single factor next return simulation"  # Output location.
...
def simulate_simple_single_factor_panel(
        ...
        signal_char_index: int = SIGNAL_CHAR_INDEX,
        signal_strength_multiplier: float = SIGNAL_STRENGTH_MULTIPLIER,
        random_state: int = RANDOM_STATE,
) -> Tuple[pd.DataFrame, List[str]]:
    """
    Simulate firm-month characteristics and a SIMPLE single-factor next return.

    The characteristics, the complex-signal RNG draws, and the noise draws are
    generated EXACTLY as in ``simulation_example.py`` (same seed -> identical
    features and identical noise). The complex conditional mean is computed only
    to (a) hold the RNG stream fixed and (b) measure its predictable variance;
    its shape is then discarded and replaced by a single-characteristic linear
    signal rescaled to that same variance.

    Rows are dated by the signal month t. The target column, ret_exc, is the
    return earned in month t + 1, generated from ``char_{signal_char_index+1}``
    known at t.

    Returns:
        A tuple of (panel_dataframe, feature_columns).
    """
    if not 0 <= signal_char_index < n_firm_chars:
        raise ValueError(
            f"signal_char_index must be in [0, {n_firm_chars}), "
            f"got {signal_char_index}"
        )
    ...
    # ------------------------------------------------------------------
    # SIMPLE-DGP OVERRIDE -- the only change versus simulation_example.py.
    #
    # Everything above is the byte-for-byte complex DGP, run purely to (a) hold
    # the RNG stream identical so the characteristics AND the noise drawn below
    # match the original economy exactly, and (b) measure the predictable
    # variance of the complex signal. We now DISCARD the complex signal's shape
    # and replace the conditional mean with a single-characteristic LINEAR
    # ("CAPM-like") signal, rescaled to that same predictable variance. So the
    # only thing that differs between the two experiments is the COMPLEXITY of
    # the true function E[ret | chars]; the features, the noise, and the R^2
    # ceiling are held fixed. char_{signal_char_index+1} is the single factor;
    # the other characteristics are independent AR(1)s, hence irrelevant.
    # ------------------------------------------------------------------
    complex_signal_scale = float(complex_conditional_mean.std())
    single_factor = standardize_terms(
        characteristics[:, :, [signal_char_index]]
    )[:, :, 0]
    conditional_mean = (
        signal_strength_multiplier * complex_signal_scale * single_factor
    )
...
def main() -> None:
    ...
    print_system_info()
    print("=" * 80)
    print("SIMPLE SINGLE-FACTOR NEXT-RETURN SCALING LAW SIMULATION")
    print("=" * 80)
    ...
    signal_col = f"char_{SIGNAL_CHAR_INDEX + 1:02d}"
    print(f"Single relevant factor: {signal_col} (linear); other 63 chars irrelevant")
    print(f"Signal strength multiplier: {SIGNAL_STRENGTH_MULTIPLIER:.2f} "
          "(1.0 = matched to the complex DGP's predictable variance)")
    print("Timing: characteristics at month t predict ret_exc earned in month t + 1")

    df, feature_cols = simulate_simple_single_factor_panel()

    panel_path = OUTPUT_DIR / "simulated_simple_single_factor_panel.csv"
    ...
    # DGP sanity: the single factor should carry essentially all the signal,
    # and every other characteristic should be ~uncorrelated with returns.
    ret = df["ret_exc"].to_numpy(dtype=np.float64)
    signal_corr = float(np.corrcoef(df[signal_col].to_numpy(dtype=np.float64), ret)[0, 1])
    other_cols = [c for c in feature_cols if c != signal_col]
    other_abs_corrs = np.array([
        abs(np.corrcoef(df[c].to_numpy(dtype=np.float64), ret)[0, 1])
        for c in other_cols
    ])
    print(
        "DGP check: "
        f"corr({signal_col}, ret)={signal_corr:+.4f} "
        f"-> implied R^2 ceiling ~= {signal_corr ** 2:.4f}; "
        f"mean |corr| of the other {len(other_cols)} chars={other_abs_corrs.mean():.4f} "
        f"(max={other_abs_corrs.max():.4f})"
    )
